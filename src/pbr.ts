...
export const fresnelSchlick = (cosTheta: number, f0: d.v3f): d.v3f => {
  'use gpu';
  return f0.add((std.sub(1, f0)).mul((1 - cosTheta) ** 5));
};
...
export const evaluateLight = (
  ...
  const specular = std.div((fresnel.mul((ndf * g))), (4 * ndotv * ndotl + 0.001));
  const kd = (std.sub(1, fresnel)).mul((1 - material.metallic));

  return (std.div((kd.mul(material.albedo)), PI).add(specular)).mul(radiance).mul(ndotl);
};
...
export const shade = (p: d.v3f, n: d.v3f, v: d.v3f): d.v3f => {
  ...
  const roughOffset =
    ...
    ).mul(material.roughness).mul(0.3);
