...
export function createPostProcessingPipelines(
  ...
  const extractBright = root
    ...
    .createGuardedComputePipeline((x, y) => {
      ...
      const uv = std.div((d.vec2f(x, y).add(0.5)), d.vec2f(dimensions));
  ...
  const fragmentMain = tgpu.fragmentFn({
    ...
    let final = color.rgb.add(bloomColor.rgb.mul(bloomParamsAccess.$.intensity));

    const centeredUV = (std.sub(uv, 0.5)).mul(2);
...
function createBlurPass(root: TgpuRoot, direction: 'horizontal' | 'vertical') {
  return root.createGuardedComputePipeline((x, y) => {
    ...
    const texelSize = std.div(1, d.vec2f(dimensions));
    const uv = std.div((d.vec2f(x, y).add(0.5)), d.vec2f(dimensions));
    ...
    for (let i = -BLUR_RADIUS; i <= BLUR_RADIUS; i++) {
      ...
      result = result.add(std.textureSampleLevel(
          processLayout.$.inputTexture,
          processLayout.$.sampler,
          uv.add(offset),
          0,
    ...
    std.textureStore(
      ...
      d.vec2u(x, y),
      d.vec4f(std.div(result, totalWeight), 1),
    );
  });
}
